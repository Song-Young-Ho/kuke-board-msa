...
class CommentServiceTest {
    ...
    void deleteShouldMarkAsDeletedWhenHasChildren() {
        Long articleId = 1L;
        Long commentId = 2L;

        Comment comment = createComment(articleId, commentId);

        given(repository.findById(commentId)).willReturn(Optional.of(comment));
        given(repository.countBy(articleId, commentId, 2L)).willReturn(2L);
    ...
    @Test
    @DisplayName("하위 댓글이 삭제되고, 삭제되지 않은 부면, 하위 댓글만 삭제한다.")
    void deleteShouldDeleteChildOnlyIfNotDeletedParent() {
        ...
        given(repository.findById(commentId)).willReturn(Optional.of(comment));
        given(repository.countBy(articleId, commentId, 2L)).willReturn(1L);

        given(repository.findById(parentCommentId))
                .willReturn(Optional.of(parentComment));

        commentService.delete(commentId);

        verify(repository).delete(comment);
        verify(repository, never()).delete(parentComment);
    }

    @Test
    @DisplayName("하위 댓글이 삭제되고, 삭제된 부모면, 제귀적으로 모두 삭제한다.")
    void deleteShouldDeleteAllRecursivelyIfDeletedParent() {
        ...
        given(repository.findById(parentCommentId)).willReturn(Optional.of(parentComment));
        given(repository.countBy(articleId, parentCommentId, 2L)).willReturn(1L);

        commentService.delete(commentId);

        verify(repository).delete(comment);
        verify(repository).delete(parentComment);
    }
